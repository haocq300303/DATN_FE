...
export const fetchAllFeedback = createAsyncThunk('feedback/fetchAllFeedback', async (_, thunkAPI) => {
  try {
...
export const fetchCreatFeedback = createAsyncThunk('feedback/fetchCreatFeedback', async (feedback: any, thunkAPI) => {
  try {
...
export const fetchUpdateFeedback = createAsyncThunk('feedback/fetchUpdateFeedback', async ({ feedback }: { feedback: any }, thunkAPI) => {
  try {
...
export const fetchDeleteFeedback = createAsyncThunk('feedback/fetchDeleteFeedback', async (idFeedback: string, thunkAPI) => {
  try {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchUpdateFeedback.fulfilled, (state, action) => {
        state.feedbacks = state.feedbacks?.map((feedback: any) => (feedback._id === action?.payload?._id ? action.payload : feedback));
        state.isLoading = false;
      })
    ...
    builder
      ...
      .addCase(fetchDeleteFeedback.fulfilled, (state, action) => {
        state.feedbacks = state.feedbacks?.filter((feedback: any) => feedback._id !== action.payload._id);
        state.isLoading = false;
      })
