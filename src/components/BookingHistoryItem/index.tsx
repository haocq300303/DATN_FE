                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-800">
                      <tr>
                        ...
                        <th
                          scope="col"
                          className="px-4 py-3.5 text-sm font-medium text-left rtl:text-right text-gray-800 dark:text-gray-400"
                        >
                          Ngày
                        </th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                      {filteredBookings.map((booking: any) => (
                        <tr key={booking.id}>
                          ...
                          <td className="px-4 py-4 text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">
                            <span className="text-gray-800">{booking.shift?.time_start}</span>-
                            <span className="text-green-800">{booking.shift?.time_end}</span>
                          </td>
                          <td className="px-4 py-4 text-sm whitespace-nowrap">
                            <div className="flex items-center gap-x-6 ">
                              {booking.status === 'success' ? (
                                <button onClick={handleCancelBooking} className={`p-3 text-sm rounded-lg bg-green-100 `}>
                                  <span className={`text-green-600 font-bold shadow-white `}>Thành Công</span>
                                </button>
                              ) : (
                                <button onClick={handleCancelBooking} className={`p-3 text-sm rounded-lg bg-red-100 `}>
                                  <span className={`text-red-600 font-bold shadow-white `}>Hủy đặt lịch</span>
                                </button>
                              )}
                            </div>
                          </td>
