...
const ModalViewCreatePitch = ({ isModalOpen, setIsModalOpen }: any) => {
  ...
  const customRequest = async ({ file, onSuccess, onError }: any) => {
    try {
      ...
      if (response?.status === 200) {
        ...
        console.log('success', onSuccess(response, file));
      } else {
          ...
          {current === steps.length - 1 && (
            <Button
              ...
              onClick={() =>
                form
                  .validateFields()
                  .then((values: any) => onFinishStep(values))
              }
            >
              Done
            </Button>
