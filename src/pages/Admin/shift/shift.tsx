...
const Shift = () => {
  ...
  const columns: ColumnsType<IShift> = [
    ...
    },
    {
      ...
      width: 140,
      render: (chil_pitch) => <span>Sân {chil_pitch?.code_chirldren_pitch}</span>,
    },
    {
      title: 'Giá',
      dataIndex: 'price',
    ...
    },
    {
      ...
      render: (find_opponent) => (
        <span className={`px-[10px] py-[4px] rounded-full text-[#fff] ${find_opponent === 'Find' ? 'bg-red-500' : 'bg-green-500'}`}>
    ...
    },
    {
      ...
      width: 160,
      render: (value) => (
        <span className={`px-[10px] py-[4px] rounded-full text-[#fff] ${value ? 'bg-red-500' : 'bg-green-500'}`}>
          {value ? 'Yes' : 'No'}
        </span>
      ),
    },
