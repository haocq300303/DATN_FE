...
const ShiftManagement = () => {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      const { data } = await getShiftDefaultByPitch("653ca30f5d70cbab41a2e5d0");
      setShifts(data.data);
      setIsLoading(false);
    })();
  }, []);
      ...
      render: (record) => (
        <Space size="middle">
          <Button
            type="primary"
            onClick={() => {
              const shift: any = shifts?.find(
                (shift: IShift) => shift._id === record._id
              );
  ...
  const onFinish = async (values: any) => {
    if (modalMode === "add") {
      ...
      const newData = shifts?.map((shift: any) =>
        shift._id === _id ? data.data : shift
      );
      setShifts(newData);
