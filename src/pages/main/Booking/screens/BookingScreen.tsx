...
const BookingScreen = ({ setCurrent }: BookingScreenProps) => {
  const infoBooking = JSON.parse(
    sessionStorage.getItem("infoBooking") as string
  ) as IInfoBooking;
  ...
  const [totalPrice, setTotalPrice] = useState<number>(
    infoBooking?.shift.price + _totalPriceService
  );
  //
  ...
  useEffect(() => {
    const handleNewBooking = async () => {
      ...
      const infoBooking: IInfoBooking = JSON.parse(
  ...
  return (
    <div className="border border-solid px-5 py-2 rounded-md border-[#e7e7e7]">
      <div className="note">
        Vui lòng xác nhận đúng thông tin và thời gian đặt sân để chúng tôi có
        thể xử lý cho bạn chính xác nhất
      </div>
      {/*  */}
      <section>
        ...
        <div className="md:flex items-center">
          ...
          <div className="mt-2 md:mt-0 flex-1 md:ml-10 space-y-2 md:space-y-4 text-base md:text-lg">
            <div className="flex flex-wrap ">
              ...
              <span className="flex-1 text-[#242424]">
                {infoBooking?.pitch?.name}
              </span>
            </div>
            <div className="flex flex-wrap ">
              <div className="mr-3 flex justify-between text-[#422eb1] font-semibold  w-[100px]">
              ...
              <span className="flex-1 text-[#242424]">
                {infoBooking?.children_pitch?.children_pitch_code}
              </span>
            </div>
            <div className="flex flex-wrap">
      ...
      <section>
        ...
        <div className="flex justify-between items-center">
          <Radio.Group
            ...
            defaultValue={1}
            value={modeBanking}
            onChange={handleChangeModeBanking}
          >
            ...
            <Radio value={2} className="text-base">
              Đặt cọc (20%) - (
              {(infoBooking?.shift?.price * 0.2)?.toLocaleString()} VNĐ)
            </Radio>
          </Radio.Group>
      ...
      <section>
        ...
        <div className="space-y-2 max-h-[210px] overflow-y-auto">
          <Show
            when={!infoBooking?.services || infoBooking?.services?.length === 0}
          >
            Bạn chưa dùng dịch vụ nào
          </Show>
          {infoBooking.services?.map((service) => (
            <div
              key={service._id}
              className="max-w-[650px] grid grid-cols-[1fr_2fr_1fr] border border-gray-500  rounded-md px-4 py-2"
            >
              <div className="w-20">
                <img
                  src={service.image}
      ...
      <section>
        ...
        <div className="text-base">
          Chi phí hủy sân là toàn bộ số tiền cọc sân trước đó
        </div>
      </section>
      ...
      <section>
        ...
        <div className="mt-4 text-sm max-w-[800px]">
          <span className="text-red-500">Lưu ý *:</span> Chúng tôi chỉ nhận
          thanh toán bằng hình thức internet banking qua trang web của chúng
          tôi. Mọi thanh toán ngoài luồng đều không được chấp nhận.
        </div>
      </section>
      ...
      <section className="my-4 flex justify-end">
        <Button
          type="primary"
          className="bg-[#36d833] text-white"
        ...
        <Button
          type="primary"
          className="ml-6 bg-[#3255f3] text-white"
